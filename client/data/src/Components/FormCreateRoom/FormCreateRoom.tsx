...
function FormCreateRoom(props: props) {
  ...
  const [defaultDirection, setdefaultDirection] = useState<number>(
    Math.random() * 2 * Math.PI
  );
  const [boardWidth, setBoardWidth] = useState<number>(0.015);
  const [boardHeight, setBoardHeight] = useState<number>(0.2);
  const [ballRadius, setBallRadius] = useState<number>(0.02);
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (roomName === "" || Owner === "") {
      setError(
        "Error: room's name, number of players and owner can't be empty."
      );
      return;
    }
    console.log(e.target.name.value);
    axios
      .post(`http://192.168.1.53:7000/api/room/createRoom`, {
        name: roomName,
        nbPlayers: NbPlayers,
        owner: Owner,
        status: Status,
      ...
      .catch((err) => {
        console.log(err);
        setError(
          "Error: room " + e.target.name.value + " already exists or not valid."
        );
      });
  };
  return (
      ...
      <form className="ChannelRoomForm" onSubmit={handleSubmit}>
        ...
        <div className="ChannelRoomFormInput-NbPlayers">
          ...
          <input
            ...
            id="NbPlayers"
            name="name"
            onChange={(e) => setNbPlayers(e.target.valueAsNumber)}
          />
        </div>
        ...
        <div className="ChannelRoomFormInput-DefaultSpeed">
          ...
          <input
            ...
            id="DefaultSpeed"
            name="name"
            onChange={(e) => setDefaultSpeed(e.target.valueAsNumber)}
          />
        </div>
        <div className="ChannelRoomFormInput-DefaultSpeed">
          ...
          <input
            ...
            id="DefaultDirection"
            name="name"
            onChange={(e) => setdefaultDirection(e.target.valueAsNumber)}
          />
        </div>
        ...
        <div className="ChannelRoomFormInput-BallRadius">
          ...
          <input
            defaultValue={ballRadius}
            type="number"
            id="BallRadius"
            name="name"
            onChange={(e) => setBallRadius(e.target.valueAsNumber)}
          />
        </div>
        <button className="FormCreateRoomButtom" type="submit">
          Create Room
        </button>
      </form>
      <div>
