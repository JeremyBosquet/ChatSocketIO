					...
					<div>
						<div className='menu'>
							<div className='ProjectName'>
								<button onClick={() => {closeSocket();navigate("/")}}> Ft_transcendance </button>
							</div>
							<div id='navButtons'>
								<div id='profile' className='click'>
									<label className='icon' htmlFor="profile"> <input type="radio" id='profile' value="Profile" name="menu_text" onChange={() => {}} onClick={() => {closeSocket();activeorDisable(0);}} checked={checkedProfile}/> <span className='span'>< FaUserCircle/></span> </label>
								</div>
								<div id='settings' className='click'>
									<label className='icon' htmlFor="settings"> <input type="radio" id='settings' value="Settings" name="menu_text" onChange={() => {}} onClick={() => {closeSocket();activeorDisable(1)}} checked={checkedSettings}/> <span className='span'><IoMdSettings/></span> </label>
								</div>
								<div id='logout' className='click'>				
									<label className='icon' htmlFor="logout"> <input type="radio" id='logout' value="Logout" name="menu_text" onChange={() => {}} onClick={() => {closeSocket();activeorDisable(2)}} checked={checkedLogout}/> <span className='span'><IoLogOutSharp/></span>  </label>
								</div>
								<div id='game' className='click'>	
									<label className='icon' htmlFor="game"> <input type="radio" id='game' value="Game" name="menu_text" onChange={() => {}} onClick={() => {closeSocket();activeorDisable(3)}} checked={checkedGame}/> <span className='span'><IoGameController/></span> </label>
								</div>
								<div id='spectate' className='click'>	
									<label className='icon' htmlFor="spectate"> <input type="radio" id='spectate' value="Spectate" name="menu_text" onChange={() => {}} onClick={() => {closeSocket();activeorDisable(4)}} checked={checkedSpectate}/> <span className='span'><BsFillEyeFill/></span> </label>
								</div>
							</div>
							<div>
							{
								(friendRequest) ?
									(
										...
										<label className='icon' htmlFor="social"> <input type="radio" id='social' value="Social" name="menu_text" onChange={() => {}} onClick={() => {closeSocket();activeorDisable(5)}} checked={checkedSocial}/><span className='span'><FaUserFriends/></span> </label>
